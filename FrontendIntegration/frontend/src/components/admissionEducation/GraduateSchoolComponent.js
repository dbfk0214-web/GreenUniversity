...
const GraduateSchoolComponent = () => {
  const graduateSchools = SupportGraduate.colleges;
  ...
  const expandAll = () => {
    const allOpen = {};
    graduateSchools.forEach((school, index) => {
      // 학과가 있는 경우에만 열기 상태로 설정
  ...
  return (
    <div className="font-sans text-[#444]">
      {/* 1. 페이지 타이틀 */}
      <div className="mb-10">{makeCommonTitle("대학원", "대학·대학원")}</div>
      ...
      <div className="space-y-10">
        {graduateSchools.map((school, idx) => {
          const isExpanded = expandedItems[idx];
          const hasDepts = school.departments && school.departments.length > 0;
              ...
              <div className="flex-1 p-6 lg:p-8 flex flex-col justify-center">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end border-b border-[#e5e5e5] pb-4 mb-5">
                  ...
                  <div className="flex gap-3 mt-2 sm:mt-0 text-sm">
                    ...
                    {/* JSON에 메인 홈페이지 링크가 없지만 디자인상 필요하다면 아래와 같이 렌더링 */}
                    <a
                      href="#none"
                      className="text-[#666] hover:text-[#002c62] flex items-center gap-1 transition-colors"
                    >
                ...
                <div className="flex flex-wrap items-end justify-between gap-4">
                  ...
                  {/* 학과 목록 토글 버튼 (학과 데이터가 있을 경우에만 표시) */}
                  {hasDepts && (
                    <button
                      ...
                      {school.departments.map((dept, dIdx) => (
                        ...
                        >
                          ...
                          <div className="flex gap-2 shrink-0">
                            ...
                            {/* 홈페이지 링크가 JSON에 없더라도 디자인상 버튼 틀은 유지하거나, 데이터가 있을 때만 렌더링 */}
                            <a
                              href={dept.homepage || "#none"}
                              target="_blank"
